...
- [Friday Labs OS — Software Manual](software/Friday Labs OS Software Manual.md) — **start here for the software.** The big-picture architecture, the workspace map, build/run + the dev workflow, the debugging toolkit, a plain-language glossary, and the foundation-phase map. Each finished phase adds a chapter.
  ...
  - [Phase 4 — Safety](software/Phase 4 - Safety.md) — **authority enforcement** (only the `authority`-lease holder may command; nonce + expiry; wrong-source/replay rejected) and the **safe-stop watchdog** (safety-pulse loss / EmergencyStop → motors-0 + steering frozen, verified at 112 ms in sim). Split-brain failover + HIL 100 ms validation are the remaining slice.
...
| Mechanical anatomy reference | HowToMechatronics Perseverance replica; 6 drive + 4 corner steer; rocker-bogie + mast; **130 mm wheel** (creator-confirmed, ≈0.247×, ~69 cm body — confirm off the in-repo DIY CAD); CAD in `Mechanics/reference-models/howtomechatronics-replica/` | [Mechanical Design Reference](addendums/Mechanical Design Reference.md) |
